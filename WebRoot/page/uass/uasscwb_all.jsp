...
$('#chutuan').attr('value',x[0].value);
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  <form action="<%=path%>/uscwviewall.action" method="post" name="fm1">
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="400px" height="25" align="center" valign="middle" nowrap><div
										align="center">${entity.content}</div></td>
								<td width="80px" height="25" align="center" valign="middle" nowrap><div
										align="center">${fb:newnumbertoname(entity.undertake)}</div></td>
								...
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										...
										<a href="<%=path%>/uscwupdatestatus.action?type=shfq&number=${entity.number}&newnumber=${newnumber}">收回</a> 
										&nbsp;&nbsp;
										</c:if>
										<c:if test="${(newnumber==entity.preundertake)&&(entity.status==2)}">
										<a href="<%=path%>/uscwupdatestatus.action?type=shcb&number=${entity.number}&newnumber=${newnumber}">收回</a> 
										&nbsp;&nbsp;
										</c:if>
										<c:if test="${(authoI=='I')&&(entity.status!=6)}">
										<a href="<%=path%>/uscwupdatestatus.action?type=cx&number=${entity.number}&newnumber=${newnumber}">撤销</a>&nbsp;&nbsp;
										</c:if>
										<a href="<%=path%>/uscwviewdetail.action?number=${entity.number}&newnumber=${newnumber}">查看详情</a> 
										</div></td>	
							</tr>
							</c:forEach>
							<tr class="表格表头背景">
							<td colspan="14">
								<div align="center">
								<a	href="<%=path%>/uscwviewall.action?zhuan=1&begindate=${begindate}&enddate=${enddate}&name=${strtemp}&newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/uscwviewall.action?zhuan=1&begindate=${begindate}&enddate=${enddate}&name=${strtemp}&newnumber=${newnumber}&currentPage=${nextPage}"
