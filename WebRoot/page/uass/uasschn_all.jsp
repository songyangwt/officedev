...
$('#chutuan').attr('value',x[0].value);
 ...
 function expressout()
	{
		...
		window.location = "<%=path%>/uschexport.action?begindate="+begindate+"&enddate="+enddate;
	}
 </script>
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  <form action="<%=path%>/uschviewall.action" method="post" name="fm1">
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										align="center">
										<c:if test="${(newnumber==entity.initiator)&&(entity.status==99)}">
										<a href="<%=path%>/uschbaocun.action?type=xinzeng&uchpid=0&number=${entity.number}&newnumber=${newnumber}">编辑</a> 
										&nbsp;&nbsp;
										</c:if>
										<c:if test="${(newnumber==entity.initiator)&&(entity.status==1)}">
										<a href="<%=path%>/uschupdatestatus.action?type=shfq&number=${entity.number}&newnumber=${newnumber}">收回</a> 
										&nbsp;&nbsp;
										</c:if>
										<c:if test="${(newnumber==entity.preundertake)&&(entity.status==2)}">
										<a href="<%=path%>/uschupdatestatus.action?type=shcb&number=${entity.number}&newnumber=${newnumber}">收回</a> 
										&nbsp;&nbsp;
										</c:if>
							...
							<td colspan="14">
								<div align="center">
								<a	href="<%=path%>/uschviewall.action?zhuan=1&begindate=${begindate}&enddate=${enddate}&chutuan=${chutuan}&name=${strtemp}&newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/uschviewall.action?zhuan=1&begindate=${begindate}&enddate=${enddate}&chutuan=${chutuan}&name=${strtemp}&newnumber=${newnumber}&currentPage=${nextPage}"
