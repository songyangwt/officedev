...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewzckfoutunder.action" method="post" name="fm1">
				<br>
					<br>	
					<table height="80" align="center" cellpadding="0"
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<c:forEach items="${list}" var="zckfout" varStatus="status">
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="125px" height="25" align="center" valign="middle" nowrap><div
										align="center">${zckfout.number}</div></td>
								<td width="60px" height="25" align="center" valign="middle" nowrap><div
										align="center">${zckfout.date}</div></td>
								<td width="120px" height="25" align="center" valign="middle" nowrap><div
										align="center">${zckfout.name}</div></td>
							
								<td width="80px" height="25" align="center" valign="middle" nowrap><div
										align="center">${fb:newnumbertoname(zckfout.thisunder)}</div></td>
								<td width="70px" height="25" align="center" valign="middle" nowrap><div
										align="center">${fb:newstatustostring(zckfout.status)}</div></td>		
								
								 
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										align="center">
										<c:choose>	
										<c:when test="${(newnumber==zckfout.initiator)&&(zckfout.status==0||zckfout.status==5)}">
												<a href="<%=path%>/deletezckfoutpage.action?number=${zckfout.number}&newnumber=${newnumber}">撤销</a>&nbsp;&nbsp;
												<a href="<%=path%>/viewzckfoutdetail.action?number=${zckfout.number}&newnumber=${newnumber}">查看详情</a> 
										</c:when>

										<c:otherwise>
										<a href="<%=path%>/checkzckfout.action?number=${zckfout.number}&newnumber=${newnumber}">复核</a> 
										&nbsp;&nbsp;&nbsp;&nbsp;
										</c:otherwise>
			                            </c:choose>	
							...
							<td colspan="14">
								<div align="center">
								<a	href="<%=path%>/viewzckfoutunder.action?newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewzckfoutunder.action?newnumber=${newnumber}&currentPage=${nextPage}"
