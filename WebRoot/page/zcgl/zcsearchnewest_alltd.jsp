...
function tijiao() {
		  ...
		  var assetchu = document.getElementById("assetchu").value;
		      ...
		      with(document.forms[0]) {
						...
						action='viewzcsearchallnewzx.action';
						method="post";
						submit();
					}
	...
	function changeall()
	{
			window.location = "<%=path%>/changealldr.jsp";
	}
   ...
   function show(name,type,chu,status)
	{
		...
		window.location = "<%=path%>/showsearchdetail.action?name="+namec+"&type="+typec+"&chu="+chuc+"&status="+statusc+"&flag=1";
	}
  ...
  <form action="<%=path%>/viewzcsearchallzx.action" method="post" name="fm1">
					...
					<table id="tab" height="80" align="center" cellpadding="0"
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="${status.index+1+(currentPage-1)*pageSize}" style="height:20px">
