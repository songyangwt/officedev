...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewzcapplyunder.action" method="post" name="fm1">
			<br>
			 <br>
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="100px" height="25" align="center" valign="middle" nowrap><div
										align="center">
										<c:choose>	
										<c:when test="${(newnumber==zcapply.initiator)&&(zcapply.status==0||zcapply.status==5)}">
										...
										<c:otherwise>
										   <c:choose>
										    <c:when test="${(zcapply.status==14)}">
												<a href="<%=path%>/viewzcapplyfennum.action?number=${zcapply.number}&newnumber=${newnumber}">分配资产编号</a>
											</c:when>
											 <c:when test="${(zcapply.status==15)}">
												<a href="<%=path%>/viewzcapplyfenname.action?number=${zcapply.number}&newnumber=${newnumber}">分配资产到人员</a>
											</c:when>
											<c:otherwise>
											<a href="<%=path%>/viewzcapplyunderdetail.action?number=${zcapply.number}">处理</a>
											</c:otherwise>
							...
							<td colspan="14">
								<div align="center">
								<a	href="<%=path%>/viewzcapplyunder.action?newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewzcapplyunder.action?newnumber=${newnumber}&currentPage=${nextPage}"
