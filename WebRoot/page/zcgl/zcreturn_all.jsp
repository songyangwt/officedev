...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewzcreturnall.action" method="post" name="fm1">
				<br>
					<br>		
					<table height="80" align="center" cellpadding="0"
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<c:forEach items="${list}" var="zcreturn" varStatus="status">
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="125px" height="25" align="center" valign="middle" nowrap><div
										align="center">${zcreturn.number}</div></td>
								<td width="60px" height="25" align="center" valign="middle" nowrap><div
										align="center">${zcreturn.date}</div></td>
								<td width="100px" height="25" align="center" valign="middle" nowrap><div
										align="center">${zcreturn.name}</div></td>
								<td width="150px" height="25" align="center" valign="middle" nowrap><div
										align="center">${fb:chutostring(zcreturn.chu)}</div></td>
								<td width="250px" height="25" align="center" valign="middle" nowrap><div
										align="center">${zcreturn.reason}</div></td>
								<td width="70px" height="25" align="center" valign="middle" nowrap><div
										align="center">${zcreturn.tel}</div></td>
								<td width="80px" height="25" align="center" valign="middle" nowrap><div
										align="center">${fb:newnumbertoname(zcreturn.thisunder)}</div></td>
								<td width="70px" height="25" align="center" valign="middle" nowrap><div
										align="center">${fb:newstatustostring(zcreturn.status)}</div></td>		
								
								 
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										align="center">
										<c:if test="${(authoM=='M')&&(zcreturn.status!=4)&&(zcreturn.status!=6)}">
										<a onClick="return confirm('确定撤销?');"  href="<%=path%>/deletezcreturnpage.action?number=${zcreturn.number}&newnumber=${newnumber}">撤销</a>
										&nbsp;&nbsp;
										</c:if>
										<a href="<%=path%>/viewzcreturndetail.action?number=${zcreturn.number}&newnumber=${newnumber}">查看详情</a>
										</div></td>	
							</tr>
							</c:forEach>
							<tr class="表格表头背景">
							<td colspan="15">
								<div align="center">
								<a	href="<%=path%>/viewzcreturnall.action?zhuan=1&begindate=${begindate}&enddate=${enddate}&chutuan=${chutuan}&name=${strtemp}&newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewzcreturnall.action?zhuan=1&begindate=${begindate}&enddate=${enddate}&chutuan=${chutuan}&name=${strtemp}&newnumber=${newnumber}&currentPage=${nextPage}"
