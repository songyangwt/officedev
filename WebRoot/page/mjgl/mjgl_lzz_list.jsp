...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewmjgllzz.action" method="post" name="fm1">
						...
						<c:forEach items="${list}" var="mjgl" varStatus="status">
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										align="center">
										<c:if test="${(newnumber==mjgl.initiator)&&(mjgl.status==1)}">
										<a href="<%=path%>/updatemjglstatus.action?number=${mjgl.number}&newnumber=${newnumber}">收回</a> 
										&nbsp;&nbsp;&nbsp;&nbsp;
										</c:if>
										<c:if test="${(newnumber==mjgl.initiator)&&(mjgl.status!=1)}">
							...
							<td colspan="14">
								<div align="center">
								<a	href="<%=path%>/viewmjgllzz.action?newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewmjgllzz.action?newnumber=${newnumber}&currentPage=${nextPage}"
