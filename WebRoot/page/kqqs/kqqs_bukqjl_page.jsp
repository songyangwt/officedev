	 ...
	 function tijiao() {
		...
		if(trim(tel)=="")
		  ...
		  else if(trim(reason)=="")
		  {
			...
			{
				alert("考勤缺失申请有效期至次月前3个工作日，如果次月前3个工作日领导仍未审批视为审批不通过，按迟到或早退记录");
...
b{
  ...
  ${daohang}
    <form action="subkqqspage.action" method="post">
       ...
       <p align="center" style="color:red">考勤缺失申请有效期至次月前3个工作日，如果次月前3个工作日领导仍未审批视为审批不通过，按迟到或早退记录</p>
        <table width="860" height="398" border="1" align="center" cellpadding="1" cellspacing="0">
    		...
    		<tr>
    			...
    			<td colspan="5">
    				${date}&nbsp;&nbsp;
    				<input type="hidden" name="qsdate" value="${date}"/>
    				<c:if test="${qdqt=='qd'}">签到</c:if>
    				<c:if test="${qdqt=='qt'}">签退</c:if>
    				<input type="hidden" name="qdqt" value="${qdqt}"/>
    			</td>
    		</tr>
    		<tr>
    			<td class="as">
