...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewkqqsunder.action" method="post" name="fm1">
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										align="center">
									<c:choose>
											<c:when test="${(newnumber==kqqs.initiator)&&(kqqs.status==0||kqqs.status==5)}">
												<a href="<%=path%>/deletekqqspage.action?number=${kqqs.number}&newnumber=${newnumber}">撤销</a>&nbsp;&nbsp;&nbsp;&nbsp;
												<a href="<%=path%>/viewkqqsdetail.action?number=${kqqs.number}&newnumber=${newnumber}">查看详情</a>  
											</c:when>
											<c:otherwise>
												<c:choose>
													<c:when test="${(kqqs.status==7)}">
														<a href="<%=path%>/viewkqqsdetail.action?number=${kqqs.number}&newnumber=${newnumber}&queren=1">确认</a>
													</c:when>
													<c:otherwise>
														<a href="<%=path%>/kqqsunderdetail.action?number=${kqqs.number}">办理</a>
													</c:otherwise>
												</c:choose>
											</c:otherwise>
										</c:choose>		
							...
							<td colspan="14">
								<div align="center">
								<a	href="<%=path%>/viewkqqsunder.action?newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewkqqsunder.action?newnumber=${newnumber}&currentPage=${nextPage}"
