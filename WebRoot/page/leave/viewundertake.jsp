...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewundertake.action?newnumber=${newnumber}" method="post" name="fm1">
					<table height="80" align="center" cellpadding="0"
						cellspacing="2" border: 0px;">
						<tr>
							<td
								style="color: #1778C2; padding-top: 0px; padding-bottom: 5px; border: 0px; font-size: 26px; font-family: '黑体';"
								colspan="11" align="center" bordercolor="#FFFFFF"><b>待办事宜</b>
							</td>
						</tr>
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<c:if test="${leave.status!=8}">
								<td width="70px" height="25" align="center" valign="middle" nowrap><div
										align="center">
									<c:choose>	
										<c:when test="${(newnumber==leave.initiator)&&(leave.status==0||leave.status==4)}">
												<a href="<%=path%>/deleteleavepage.action?number=${leave.number}&newnumber=${leave.initiator}">撤销</a>
										</c:when>
										<c:when test="${leave.status==7}">
											<a href="<%=path%>/leavedetail.action?number=${leave.number}&xiaojia=1">销假</a>
										</c:when>
										<c:otherwise>
											<a href="<%=path%>/viewundertakedetail.action?newnumber=${newnumber}&number=${leave.number}">处理</a>
										</c:otherwise>
									</c:choose>	
										</div></td>		
								</c:if> 	
							...
							<td colspan="12">
								<div align="center">
								<a	href="<%=path%>/viewundertake.action?newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewundertake.action?newnumber=${newnumber}&currentPage=${nextPage}"
