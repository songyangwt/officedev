...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewapply.action?status=1" method="post" name="fm1">
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										align="center">
										<c:if test="${(newnumber==leave.applicant)&&(leave.status==7)}">
										<a href="<%=path%>/leavedetail.action?number=${leave.number}&xiaojia=1">销假</a>
										&nbsp;&nbsp;&nbsp;&nbsp;
										</c:if>
										<c:if test="${(newnumber==leave.initiator)&&(leave.status==9)}">
										<a href="<%=path%>/updatestatus.action?number=${leave.number}&type=5&newnumber=${newnumber}">收回</a>
										&nbsp;&nbsp;&nbsp;&nbsp; 
										</c:if>
										<c:if test="${(newnumber==leave.initiator)&&(leave.status==0||leave.status==4)}">
							...
							<td colspan="12">
								<div align="center">
								<a	href="<%=path%>/viewapply.action?type=${type}&newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewapply.action?type=${type}&newnumber=${newnumber}&currentPage=${nextPage}"
