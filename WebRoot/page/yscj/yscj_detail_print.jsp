...
<script type="text/javascript">
     function print()
     {
     document.getElementById("print").ExecWB(6,1);
     }
     function printview()
     {
   ...
   // document.body.innerHTML = headstr+newstr+footstr; 
     ...
     document.getElementById("print").ExecWB(7,1);
        ...
        <table width="1160" height="398" border="1" align="center" cellpadding="1" cellspacing="0">
    		...
    		<!--<tr>
    		    <td class="as" colspan="2">
    		      非节假日出国（境），请勾选此项并附请假审批表
    		    </td>
    		    <td class="as">
    		       <input id="notholiday" type="radio" name="notholiday" value="1"/>
    		    </td>
    		    ...
    		    <c:if test="${ty.notholiday==1}">
				   	<a href="<%=path%>/leavedetail.action?number=${ty.leavepagenumber}">查看详情</a>			
    			</c:if>
    	   ...
    	   <tr>-->
    		...
    		<tr>
    			...
    			<td colspan="5">
    				${ty.begindate}<!--<c:if test="${ty.halfday=='0'||ty.halfday=='2'}">上午</c:if><c:if test="${ty.halfday=='1'||ty.halfday=='3'}">下午</c:if>&nbsp;-->，预计至
      				${ty.enddate}<!--<c:if test="${ty.halfday=='2'||ty.halfday=='3'}">上午</c:if><c:if test="${ty.halfday=='0'||ty.halfday=='1'}">下午</c:if>&nbsp;-->，共
      				${ty.sumday}天
    			</td>
    			</td>
...
VIEWASTEXT>-->
    				...
    				<!-- <input type="button" onclick="javascript:windows.print();" value="打 印" />-->
    				<object id="print" height="0"  width="0" classid="CLSID:8856F961-340A-11D0-A96B-00C04FD705A2" > </object>
    				<c:if test="${authoK=='K'}">
    				
    				
    				
    				<input type="button" onclick="printview()" value="打印预览" />
    				<!--<input type="button" onclick="javascript:wb.ExecWB(8,1);" value="页面设置" />-->
    				</c:if>
    				<input type="button" onclick="javascript:history.go(-1);" value="返  回" />
    			</td>
