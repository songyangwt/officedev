...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewyscjunder.action" method="post" name="fm1">
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										align="center">
										<c:choose>	
										<c:when test="${(newnumber==yscj.initiator)&&(yscj.status==0||yscj.status==5)}">
                                        ...
                                        <c:when test="${yscj.status==8}">
											<a href="<%=path%>/yscjtzdetail.action?number=${yscj.number}&newnumber=${newnumber}">填写护照领取时间</a>
										</c:when>
										<c:when test="${yscj.status==9}">
											<a href="<%=path%>/yscjtzdetail.action?number=${yscj.number}&newnumber=${newnumber}">填写护照归还时间</a>
										</c:when>
										<c:when test="${(yscj.status!=8)&&(yscj.status!=9)&&(yscj.status!=4)}">
											<a href="<%=path%>/yscjunderdetail.action?number=${yscj.number}">处理</a>
										</c:when>
										<c:otherwise>
							...
							<td colspan="14">
							<div align="center">
								<a	href="<%=path%>/viewyscjunder.action?newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewyscjunder.action?newnumber=${newnumber}&currentPage=${nextPage}"
