...
$('#chutuan').attr('value',x[0].value);
 ...
 function expressout()
	{
		...
		window.location = "<%=path%>/exportzzzm.action?begindate="+begindate+"&enddate="+enddate+"&chutuan="+chutuan+"&name="+name;
	}
 </script>
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewzzzmall.action" method="post" name="fm1">
						...
						<tr>
							<td colspan="26" height="10px">
								<div align="center">
									...
									<c:if test="${authoJ=='J'||authoI=='I'||chu=='1'||zhi=='0'||zhi=='1'||zhi=='2'}">
					...
					<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="100px" height="25" align="center" valign="middle" nowrap><div
										align="center">
										<c:if test="${(authoI=='I'||authoJ=='J'||authoK=='K')&&(zzzm.status!=6)}">
										<a onClick="return confirm('确定撤销?');"  href="<%=path%>/deletezzzmpage.action?number=${zzzm.number}&newnumber=${newnumber}">撤销</a>
										&nbsp;&nbsp;
										</c:if>
										<a href="<%=path%>/viewzzzmdetail.action?number=${zzzm.number}&newnumber=${newnumber}">查看详情</a>
										</div></td>	
							</tr>
							</c:forEach>
							<tr class="表格表头背景">
							<td colspan="14">
								<div align="center">
								<a	href="<%=path%>/viewzzzmall.action?zhuan=1&begindate=${begindate}&enddate=${enddate}&chutuan=${chutuan}&zxtd=td&name=${strtemp}&newnumber=${newnumber}&currentPage=${previousPage}&authoI=${authoI}&authoJ=${authoJ}&authoG=${authoG}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewzzzmall.action?zhuan=1&begindate=${begindate}&enddate=${enddate}&chutuan=${chutuan}&zxtd=td&name=${strtemp}&newnumber=${newnumber}&currentPage=${nextPage}&authoI=${authoI}&authoJ=${authoJ}&authoG=${authoG}"
