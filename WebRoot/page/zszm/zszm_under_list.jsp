...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewzzzmunder.action" method="post" name="fm1">
					...
					<tr height="50px" class="表格表头背景1" id="hang">
								...
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
									...
									<c:choose>	
										<c:when test="${(newnumber==zzzm.initiator)&&(zzzm.status==0||zzzm.status==5)}">
												<a href="<%=path%>/deletezzzmpage.action?number=${zzzm.number}&newnumber=${newnumber}">撤销</a>&nbsp;&nbsp;
										...
										<c:otherwise>
										   <c:choose>
										    <c:when test="${(zzzm.status==7)}">
												<a href="<%=path%>/viewzzzmdetail.action?number=${zzzm.number}&newnumber=${newnumber}&queren=1">确认</a>
											</c:when>
											<c:otherwise>
											<a href="<%=path%>/zzzmunderdetail.action?number=${zzzm.number}">处理</a>
											</c:otherwise>
							...
							<td colspan="14">
								<div align="center">
								<a	href="<%=path%>/viewzzzmunder.action?newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewzzzmunder.action?newnumber=${newnumber}&currentPage=${nextPage}"
