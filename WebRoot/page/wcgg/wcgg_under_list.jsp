...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewwcggunder.action" method="post" name="fm1">
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										align="center">
										<c:choose>	
										<c:when test="${(newnumber==wcgg.initiator)&&(wcgg.status==0||wcgg.status==5)}">
										...
										<c:when test="${wcgg.status==3}">
											<a href="<%=path%>/viewwcggdetail.action?number=${wcgg.number}&newnumber=${newnumber}">报到</a>
										</c:when>
										<c:otherwise>
											<a href="<%=path%>/wcggunderdetail.action?number=${wcgg.number}">处理</a>
										</c:otherwise>
									</c:choose>	
										</div></td>	
							</tr>
							...
							<td colspan="14">
								<div align="center">
								<a	href="<%=path%>/viewwcggunder.action?newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewwcggunder.action?newnumber=${newnumber}&currentPage=${nextPage}"
