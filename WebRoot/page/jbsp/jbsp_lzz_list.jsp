...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewjbsp.action" method="post" name="fm1">
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										align="center">
										<c:if test="${(newnumber==jbsp.initiator)&&(jbsp.status==1)}">
										<a href="<%=path%>/updatejbspstatus.action?number=${jbsp.number}&newnumber=${newnumber}">收回</a> 
										&nbsp;&nbsp;
										</c:if>
										<c:if test="${(newnumber==jbsp.initiator)&&(jbsp.status==0||jbsp.status==5)}">
							...
							<td colspan="14">
								<div align="center">
								<a	href="<%=path%>/viewjbsp.action?newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewjbsp.action?newnumber=${newnumber}&currentPage=${nextPage}"
