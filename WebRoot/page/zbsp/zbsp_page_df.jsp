		 ...
		 // var panduan= sumdate.selectedIndex;
		  ...
		  else if(trim(reason)=="")
		  {
		  ...
		  else if(xuanze=="wu")
			{
				...
				alert("值班开始时间不能晚于值班结束时间！");return;
	     }
		 else if(((enddate-begindate)/(1000 * 60 * 60*24)+1)<(sumdate)){
				document.getElementById("begindate").value="";
				document.getElementById("enddate").value="";
				alert("开始时间与结束时间之差不应小于值班时间");return;
	     }
			else
			{
				if (window.confirm(message)) {
					with(document.forms[0]) {
						action='subzbsppage.action';
						method="post";
						submit();
					}
				}
...
var xmlhttp;
	...
	xmlhttp.open("GET","showzbsppagedfajax.action?name="+name+"&newnumber="+newnumber,true);
	xmlhttp.send();
}
...
b{
  ...
  <body>
  ${daohang}
    <form action="subzbsppage.action" method="post">
    	  <p align="center" style="padding:0px;margin:0px; font-size: 24px;"><strong><font id="bu" style="display:none;">（补）</font>业务处理中心值班审批备案表</strong><strong> </strong></p>
       <table width="860" height="398" border="1" align="center" cellpadding="1" cellspacing="0">
    		...
    		<tr>
    			<td>
    				<input type="button" id="button" value="增加一位值班人" onclick="a()"/>
    			</td>
    			<td colspan="4" id="p">&nbsp;
    			</td>
    			<td><input type="button" id="button" value="减少一位值班人" onclick="b()"/></td>
    		</tr>
    		<tr>
    			<td class="as">
    				值班事由<b>*</b>
    		...
    		<tr>
    			...
    			<td colspan="5">
    				...
    				<input type="hidden" id="allname" name="allname" value=""/>
    				<input type="button" onclick="tijiao()" value="提  交"/>
    				<span class="tip">（本次值班产生的调休将于6个月后失效！）</span>
