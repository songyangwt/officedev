...
public class ImportMjglPage {
	
	private static Logger logger = Logger.getLogger(ImportMjglPage.class);
...
public String execute() throws Exception {
		...
		if(yearmonth.compareTo("201501")<0||yearmonth.compareTo("209912")>0)
		{
	    	...
	    	TMjglPageDAO kidao = new TMjglPageDAO();
	    	try {
				...
				String sql = "truncate t_mjgl_page";
				session.createSQLQuery(sql).executeUpdate();
				for (int i = 1; i < nn; i++) {
					
					TMjglPage ki = new TMjglPage();
					ki.setName(sheet.getCell(1, i).getContents().trim());
					if(ki.getName().equals("楮永娴")||ki.getName().equals("禇永娴"))
					{
						ki.setName("褚永娴");
					}
					if(ki.getName().equals("虞昕昀"))
					{
						ki.setName("虞昕韵");
					}
					String chu = sheet.getCell(2, i).getContents().trim();
				    ki.setChu(chu);
				    ki.setCurrentauth(sheet.getCell(4, i).getContents().trim());
				    ki.setCurrenttime(Integer.parseInt(sheet.getCell(5, i).getContents().trim()));
