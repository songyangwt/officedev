...
public class ViewZcReturnAll {
	private static final Log log = LogFactory.getLog(AssetReturn.class);
	private String newnumber;// 新一代员工编号
	private List<AssetReturn> list;
	...
	*/
	...
	public List<AssetReturn> getList() {
		return list;
	}

	public void setList(List<AssetReturn> list) {
		this.list = list;
	}
	...
	public String execute() throws Exception
	{
		...
		try {
			...
			if(zxtd!=null&&zxtd.equals("td"))
			{
				...
				{
					hql += " and ay.chu='"+chutuan+"'";
				}
				else if(chutuan>=7&&chutuan<=9)
				{
					int temptuan = chutuan-6;
					hql += " and ay.chu='3' and ay.zu='"+temptuan+"'";
				}
				else if(chutuan>=10&&chutuan<=12)
				{
					int temptuan = chutuan-9;
					hql += " and ay.chu='6' and ay.zu='"+temptuan+"'";
				}
				
				if(zhi.equals("0")||zhi.equals("1")||autho.contains("P")||autho.contains("M"))//主任看全部
				{
					hql += "";
				}
				...
				{
					hql += " and ay.chu='"+chu+"' and ay.zu='"+zu+"'";
				}
				result = "successtd";
			}
			else//普通员工
			{
				...
				hql += " and substr(ay.initiator,1,8)='"+newnumber1+"'";
			}
			hql += " order by ay.id desc";
