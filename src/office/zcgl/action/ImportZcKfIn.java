	       ...
	       message = "传入文件有误！";
	    	...
	    	try {
				...
				for (int i = 1; i < nn; i++) {
					...
					if((!sheet.getCell(2, i).getContents().trim().equals(""))&&(sheet.getCell(2, i).getContents().trim()!=null))
				    {
					   ai = aidao.findAllByNumber(assetnumber);
				    }
					...
					else if(sheet.getCell(2, i).getContents().trim().equals("")||sheet.getCell(2, i).getContents().trim()==null)
					{
						flag = 3;
						break;
					}
				  ...
				  {
					  message="资产编号重复！";
					  //ActionContext.getContext().put("message", "资产编号重复！");
						 result="failed";
				  }
