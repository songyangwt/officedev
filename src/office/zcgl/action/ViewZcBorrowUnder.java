...
public class ViewZcBorrowUnder {
	...
	*/
	...
	public List<AssetBorrow> getList() {
		return list;
	}

	public void setList(List<AssetBorrow> list) {
		this.list = list;
	}
	...
	public String execute() throws Exception
	{
		...
		try {
			...
			//hql1="from AssetBorrow as ay where ay.status=14 and ay.initiator!='"+newnumber+"'order by ay.id desc";
			hql2= "select * from asset_borrow as ay where substr(ay.thisunder,1,8)='"+newnumber1+"' or (substr(ay.initiator,1,8)='"+newnumber1+"' and ay.status in (0,5)) order by ay.id desc";
			...
			/*if(newnumber.equals("66191663"))
			{
				...
				totalRows = session.createQuery(hql1).list().size();
			}*/
		
				query = session.createSQLQuery(hql2).addEntity(AssetBorrow.class);
				//query.setFirstResult(pageSize * (currentPage - 1));
				//query.setMaxResults(pageSize);
				//totalRows = session.createQuery(hql2).list().size();
