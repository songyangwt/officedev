...
public class ViewZcSearchAllNewZong {
	...
	public String execute() throws Exception
	{
 	    ...
 	    if(assettype!=null&&!assettype.equals("wu")&&zhuan==1&&!assettype.equals(""))
		{
 	    ...
 	    hqllist="select ai.name,ai.type,ai.chu,ai.status,count(ai.name) from asset_info as ai group by ai.name, ai.type,ai.chu,ai.status having ai.chu in (0,1)";
 	    if(iswupin==1)
	    {
	    	hqllist = "select ai.name,ai.type,ai.chu,ai.status,count(ai.name) from asset_info as ai where ai.iswupin=1 group by ai.name, ai.type,ai.chu,ai.status having ai.chu in (0,1)";
	    }
	    if(iswupin==2)
	    {
	    	hqllist = "select ai.name,ai.type,ai.chu,ai.status,count(ai.name) from asset_info as ai where ai.iswupin=0 group by ai.name, ai.type,ai.chu,ai.status having ai.chu in (0,1)";
	    }
	   ...
	   String sql4 = "select ab.name,ab.chu,count(ab.chu) from ("+hqllist+" ) as ab group by ab.name,ab.type,ab.chu order by ab.name,ab.type,ab.chu,ab.status ";
		...
		{	
 	    ...
 	    hql = "select distinct(ai.name) from AssetInfo as ai where ai.chu in (0,1) order by ai.id";
