...
public class ViewZcSearchAllNew {
	...
	public String execute() throws Exception
	{
 	    ...
 	    if(assettype!=null&&!assettype.equals("wu")&&zhuan==1&&!assettype.equals(""))
		{
 	    ...
 	    hqllist="select ai.name,ai.type,ai.chu,ai.status,count(ai.name) from asset_info as ai group by ai.name, ai.type,ai.chu,ai.status having ai.chu='"+chu+"'";
 	    if(iswupin==1)
	    {
	    	hqllist = "select ai.name,ai.type,ai.chu,ai.status,count(ai.name) from asset_info as ai where ai.iswupin=1 group by ai.name, ai.type,ai.chu,ai.status having ai.chu='"+chu+"'";
	    }
	    if(iswupin==2)
	    {
	    	hqllist = "select ai.name,ai.type,ai.chu,ai.status,count(ai.name) from asset_info as ai where ai.iswupin=0 group by ai.name, ai.type,ai.chu,ai.status having ai.chu='"+chu+"'";
	    }
