...
public class ImportChangeInfoWai {
	...
	public String getAuthoS() {
		return authoS;
	}
	public void setAuthoS(String authoS) {
		this.authoS = authoS;
	}
	public File getFile() {
		return file;
		...
		if(yearmonth.compareTo("20150101")<0||yearmonth.compareTo("20991231")>0)
		{
	    	...
	    	try {
				...
				for (int i = 1; i < nn; i++) {
					  ...
					  //  AssetInfo ki = new AssetInfo();
				        ki=kidao.findAllByNumberAndAutho(sheet.getCell(0, i).getContents().trim(),authoR,authoS);
				        if(sheet.getCell(0, i).getContents().trim()!=null&&!sheet.getCell(0, i).getContents().trim().equals(""))
				        {
				        	if(ki==null)
				        	{
				        		flag=1;
					        	message="要修改的资产不存在或不属于此外包公司";
					        	break;
				        	}
