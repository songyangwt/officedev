...
public class UschViewAll {
	private static final Log log = LogFactory.getLog(UschViewAll.class);
	...
	*/
	...
	public String execute() throws Exception
	{
		...
		try {
			...
			hql = "select * from t_uass_cost_hn as up";
			...
			hql +=" where up.number in (select number from t_uass_cost_hn_people where name like '%"+strtemp+"%')";
			...
			query = session.createSQLQuery(hql).addEntity(UassCostHn.class);
			query.setFirstResult(pageSize * (currentPage - 1));
			query.setMaxResults(pageSize);
			totalRows = session.createSQLQuery(hql).addEntity(UassCostHn.class).list().size();
			initPageProperties();
			List<UassCostHn> listtemp = query.list();
			for(int i=0;i<listtemp.size();i++)
			{
				UassCostHn up = listtemp.get(i);
