...
//			} else {
				...
				index = "000"+fraw.readandwrite("ZZZM");
				index = index.substring(index.length() - 3, index.length());
				number = date + "ZZZM" + dai + index;
//			}
			//是否表中有重复项
			...
			if(dai==1)
			{
	 	    ...
	 	    tz.setRemark(remark);
	 	    tz.setIsdeparture(isdeparture);
	 	    if(yscjchoose!=null&&yscjchoose!="")
	 	    {
	 	    	tz.setYscjnumber(yscjchoose);
	 	    }
	 	    if(yscjchoose==null||yscjchoose=="")
	 	    {
	 	    	tz.setYscjnumber("wu");
	 	   ...
	 	   if(licenceen==null)
	 	    ...
	 	    tz.setLicencech(licencech);
	 	    tz.setLicenceen(licenceen);
