	...
	public String execute() throws Exception
	{
		...
		Session session = HibernateSessionFactory.getSession();
 		...
 		try {
 			...
 			Process p = pdao.findByItemAndApplicant("ZZZM", tz.getProcess());
 			String proc = p.getProcess();//完整请假流程
 			String jindu = tz.getJindu();
 			String under = tz.getThisunder();//当前审批人
 			ol.setItem("ZZZM");
 			...
 			if((thisnewnumber.substring(0,8)).equals(under))
 			{
 				...
 				if((proc.length()-jindu.length())==1)//BOSS审批完毕，进入待报道状态
 				{
 					tz.setJindu(proc);
 					tz.setStatus(4);
 					tz.setThisunder(null);//下一个处理人是申请人自己
 					//wpdao.submitWcggPage(number);//更新汇总表
 					ol.setOperate("审批完毕,"+LeaveUtil.NewNumberToNameNoSession(tz.getApplicant())+"已更新");
 					oldao.merge(ol);
 				}
 				else
 				{
 					tz.setThisunder(xuanze);
 					tz.setJindu(tz.getJindu()+thisunder);
 					tz.setStatus(2);//流转中状态
 				}
 				tz.setPreunder(thisnewnumber);
 				}
 				else
 				{
 				lpro.setOpinion(2);
 				tz.setStatus(5);//修改为已退回状态
 				tz.setPreunder(null);
 				tz.setThisunder(null);//下一处理人是自己
 				tz.setJindu(tz.getJindu().substring(0,1));
 				}
