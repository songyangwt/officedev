...
public class Importzbsp {
	private static Logger logger = Logger.getLogger(Importzbsp.class);
	...
	public String execute() throws Exception{
		...
		TZbspPageDAO jbdao = new TZbspPageDAO();
		TZbspPageDiDAO jbddao = new TZbspPageDiDAO();
    	...
    	try {
			...
			for (int i = 1; i < nn; i++) {
				...
				List<TZbspPage> listjb = jbdao.findAllByNameAndDate(name, date);
				if(!listjb.isEmpty())
			...
			if(message.equals("导入失败"))
			{
				...
				for (int i = 1; i < nn; i++) {
					...
					if(ui!=null)
					{
						...
						int halfday = 0;
						double days = 0;
						String index = "000"+fraw.readandwrite("zbsp");
						index = index.substring(index.length() - 3, index.length());
						String number = du.getStringDate() + "zbsp10" + index;
						if(sxw.contains("上午"))
						{
							halfday = 2;
							days = 0.5;
						...
						TZbspPage jp = new TZbspPage();
						TZbspPageDi jpd = jbddao.findAllByNumberName(number, name);
						jp.setNumber(number);
