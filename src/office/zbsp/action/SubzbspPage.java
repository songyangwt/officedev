...
public class SubzbspPage {
	...
	public String execute() throws Exception {
		...
		List<TZbspPage> list = jpdao.findAllByDate(date);
...
//		if (list.isEmpty()) {
//			number = date + "zbsp" + dai + bu + "001";
//		} else {
			//zbspPage lastlp = list.get(list.size() - 1);
			//String tempnumber = lastlp.getNumber();
			index = "000"+fraw.readandwrite("zbsp");
			//index = "000" + String.valueOf(list.size()+1);
			index = index.substring(index.length() - 3, index.length());
			number = date + "zbsp" + dai + bu + index;
//		}
		...
		process = gpbp.getzbspProcess(position);
		if (process == 0) {
			result = "nozbsp";
